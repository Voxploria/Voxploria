...
void VulkanRenderer::createInstance(SDL_Window* window) {
    ...
    if (!SDL_Vulkan_GetInstanceExtensions(window, &sdlExtensionCount, nullptr))
        throw std::runtime_error("Could not get required instance extensions from SDL.");
...
void VulkanRenderer::createQuadVertexBuffer() {
    ...
    Vertex         vertices[] = {{{-0.5f, -0.5f}}, {{0.5f, -0.5f}}, {{0.5f, 0.5f}}, {{-0.5f, -0.5f}}, {{0.5f, 0.5f}}, {{-0.5f, 0.5f}}};
    ...
    createBuffer(bufferSize, VK_BUFFER_USAGE_VERTEX_BUFFER_BIT | VK_BUFFER_USAGE_TRANSFER_DST_BIT, VK_MEMORY_PROPERTY_DEVICE_LOCAL_BIT, quadVertexBuffer, quadVertexBufferMemory);
...
void VulkanRenderer::createTextureImage() {
    ...
    stbi_uc* pixels = stbi_load("assets/textures/tiles.png", &texWidth, &texHeight, &texChannels, STBI_rgb_alpha);
    if (!pixels)
...
void VulkanRenderer::createImage(uint32_t width, uint32_t height, VkFormat format, VkImageTiling tiling, VkImageUsageFlags usage, VkMemoryPropertyFlags properties, VkImage& image,
